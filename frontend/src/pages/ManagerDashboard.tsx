...
export function ManagerDashboard() {
  ...
  if (loadingTeam || loadingAttendance || loadingLeave || loadingAllLeaves) {
    return (
  ...
  const teamStatus = teamMembers?.map((member) => {
    const todayAttendance = attendance?.todaysAttendance.find((record) => record.employee_name === member.first_name + ' ' + member.last_name);
    ...
    const leaveRecord = allLeaves?.find(req =>
      req.employee_name === member.first_name + ' ' + member.last_name &&
      req.status.includes('APPROVED') &&
      req.start_date <= todayStr &&
      req.end_date >= todayStr
    );
    ...
    if (leaveRecord) {
      isLeave = true;
      status = `On Leave (${leaveRecord.leave_type.replace('_', ' ')})`;
    ...
    return {
      ...
      role: member.role_details?.title || 'Employee',
      status,
      time,
      avatar: `https://ui-avatars.com/api/?name=${member.first_name}+${member.last_name}&background=random`,
      isLate,
      isLeave
    };
  }) || [];
